...
use super::Parse;
use crate::types::{Label, Line, Operation};
...
impl<'a> Parse<'a> for Line<'a> {
    fn parse_assembler(input: Span<'a>) -> LocatedIResult<'a, Self> {
        delimited(
            space0,
            tuple((
                opt(terminated(Token::<Label>::parse_assembler, space1)),
                Operation::parse_assembler,
            )),
            comment_or_space,
        )(input)
        .map(|(remainder, (label, operation))| (remainder, Self::new(label, operation)))
        .map_err(|e| match e {
...
mod tests {
        ...
        for (input, instruction_column, operand_column) in inputs_outputs.into_iter() {
            assert_eq!(
                ...
                Line::new(
                    ...
                    Operation::new(
                        Token::new(
                            Position::new(1, instruction_column),
    ...
    fn should_parse_with_comment() {
        ...
        {
            let label =
                label_column.map(|column| Token::new(Position::new(1, column), "LOOP".into()));
            assert_eq!(
                Line::parse_assembler(input.into()).unwrap().1,
                Line::new(
                    label,
